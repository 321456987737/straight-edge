...
const BarberCardSkeleton = () => (
  <div className="bg-white rounded-lg border min-h-screen border-gray-200 shadow-sm overflow-hidden">
...
export default function ServicesPage() {
  ...
  // Infinite scroll
  const lastBarberRef = useCallback(
    (node) => {
      if (loading) return;
  ...
  // Fetch barbers by service
  useEffect(() => {
    const fetchBarbersByService = async () => {
      if (!pagination.hasMore && pagination.skip > 0) return;

      setLoading(true);
      setError("");
      try {
        const searchParams = new URLSearchParams();
        if (selectedService) searchParams.append("specialty", selectedService); // Changed from "service" to "specialty"
        ...
        if (res.data && res.data.Barbers) {
          ...
          setResults((prev) => {
            if (pagination.skip === 0) return newBarbers;
        ...
        if (pagination.skip === 0) setResults([]);
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [selectedService, pagination.skip, pagination.limit, pagination.hasMore]);
        ...
        {selectedService && (
          <div className="mb-6">
            <p className="text-lg font-medium text-gray-700">
              Showing barbers specializing in: 
              <span className="font-bold ml-2">{selectedService}</span>
            </p>
            <button
          ...
          <>
            {!loading && results.length > 0 && (
              <p className="text-gray-600 mb-4">
                {pagination.total} barber
                {pagination.total !== 1 ? "s" : ""} found for {selectedService}
              </p>
            )}
            
            {loading && pagination.skip === 0 ? (
              <SearchResultsSkeleton />
            ) : (
              <>
                <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                  {results.map((barber, idx) =>
                    renderBarberCard(barber, idx, results)
                  )}
                </div>
                {loading && (
            ...
            {!loading && !error && results.length === 0 && (
              <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <h3 className="mt-2 text-lg font-medium text-gray-900">
                  No barbers found for {selectedService}
                </h3>
        ...
        {!selectedService && (
          <div className="text-center py-16 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <h3 className="text-xl font-medium text-gray-900 mb-2">
              Select a service to find barbers
            </h3>
            <p className="text-gray-600 max-w-md mx-auto">
              Choose from our list of services above to find barbers who specialize in that particular style
            </p>
          </div>
        )}
      </div>
