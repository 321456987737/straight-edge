...
const TotalUsers = () => {
  ...
  const fetchData = async (type) => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/admin/customer/totalcustomer?filter=${type}`);
      setData(res.data.customersData); // ✅ use customersData now
      console.log(res.data.customersData);
    } catch (error) {
  ...
  return (
    <div className="bg-white max-w-7xl text-black rounded-2xl p-4 sm:p-6 w-full overflow-hidden  ">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <h2 className="text-lg sm:text-xl font-bold">Total Customers</h2> {/* ✅ Updated */}
        ...
        <div className="w-full h-[250px] sm:h-[350px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <Line
                type="monotone"
                dataKey="customers" // ✅ Updated to customers
                stroke="#000000"
                strokeWidth={3}
                dot={{ r: 5, fill: "#000000" }}
                activeDot={{ r: 8 }}
