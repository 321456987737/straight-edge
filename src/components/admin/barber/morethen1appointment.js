...
export default function BarberPerformance() {
  const [chartData, setChartData] = useState([
    { name: "10+ Appointments", value: 0 },
    { name: "<10 Appointments", value: 0 },
  ]);
  const [barbers, setBarbers] = useState({ tenPlus: [], lessTen: [] });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchBarbers = async () => {
      try {
        const res = await axios.get("/api/admin/barber/morethen1appointment");
        // API should return { chart, barbers }
        ...
        <div className="lg:w-1/2 flex flex-col justify-center items-center gap-4">
          <div>
            <h3 className="font-semibold text-black mb-2">
              10+ Appointments ({barbers.tenPlus.length})
            </h3>
            <ul className="space-y-1 max-h-32 overflow-y-auto">
              {barbers.tenPlus.map((b) => (
                <li key={b._id} className="text-sm text-gray-700">
                  {b.username || "Unnamed Barber"} — {b.totalAppointments} appts
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <h3 className="font-semibold text-black mb-2">
              &lt;10 Appointments ({barbers.lessTen.length})
            </h3>
            <ul className="space-y-1 max-h-32 overflow-y-auto">
              {barbers.lessTen.map((b) => (
                <li key={b._id} className="text-sm text-gray-700">
                  {b.username || "Unnamed Barber"} — {b.totalAppointments} appts
                </li>
              ))}
